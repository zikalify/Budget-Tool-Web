...
// Currency list mirroring the Android app's WorldCurrencyChooser, which lists
// java.util.Currency.getAvailableCurrencies() with the platform display name.
// The web previously asked the device (Intl.DisplayNames) for each name, but
// devices with older/slimmer locale data echo the code itself ("UMS — UMS").
// Pinning the English CLDR names here keeps every currency with its full name
// identical on all devices. XTS (testing) and XXX (no currency) are not real
// money and are left out, same as the Android list.
...
const supportedCurrencies = () => Object.keys(CURRENCY_NAMES).sort();
const currencyName = code => {
  if (code === 'NONE') return 'No currency symbol';
  if (Object.hasOwn(CURRENCY_NAMES, code)) return CURRENCY_NAMES[code];
